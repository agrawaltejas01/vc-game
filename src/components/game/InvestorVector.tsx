            ...
            <div key={key}>
              <div className="flex justify-between items-center text-xs mb-1">
                ...
                <div className="flex items-center space-x-1">
                  <span className="font-bold text-black">{Math.round(value)}%</span>
      ...
      {qualitative_insights.length > 0 && (
        <div className="card-accent-cyan relative">
          {isFeatureEnabled('showNewInsightBadges') && (
            <Badge text="NEW" variant="new" color="primary" className="absolute top-3 right-3" />
          )}
          <h4 className="font-bold text-black mb-3 text-sm">Emerging Insights</h4>
          <ul className="space-y-2">
            {qualitative_insights.map((insight, idx) => (
              <li key={idx} className="text-xs text-gray-700 flex items-start animate-slide-in-left" style={{ animationDelay: `${idx * 100}ms` }}>
